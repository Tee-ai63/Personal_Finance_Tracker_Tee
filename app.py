...
# --- Functions ---
def add_record(user_id, record_type, category, amount):
    """Insert a new transaction linked to a user"""
...
def fetch_transactions(user_id, start_date=None, end_date=None):
    ...
    query = supabase.table("transactions").select("*").eq("user_id", user_id).order("date", desc=True)
...
def export_to_pdf(summary, transactions, fig=None):
    """Export finance summary + transactions to PDF"""
    buffer = BytesIO()
...
# --- Streamlit UI ---
st.title("Personal Finance Tracker with Login")
...
if st.session_state.user is None:
    ...
    if st.button("Add Record"):
        if category and amount > 0:
            response = add_record(user_id, record_type, category, amount)
            if response.data:
    ...
    if st.button("Show Summary & Transactions"):
        ...
        rows = fetch_transactions(user_id, start_date_str, end_date_str)
